...
- Use local harness capabilities before asking the user for logs or repository state that the workspace can reveal.\n",
    ...
    context.push_str("\nCapability manifest:\n");
    if request.execution_contract.capability_manifest.is_empty() {
        context.push_str("- No additional capability metadata was supplied.\n");
    } else {
        for line in &request.execution_contract.capability_manifest {
            context.push_str("- ");
    ...
    context.push_str("\nCompletion contract:\n");
    if request.execution_contract.completion_contract.is_empty() {
        context.push_str("- Complete the turn once the current evidence is sufficient.\n");
    } else {
        for line in &request.execution_contract.completion_contract {
            context.push_str("- ");
            context.push_str(line);
            context.push('\n');
        }
