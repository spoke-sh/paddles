...
pub use self::worker_runtime::{
    ...
    WorkerRuntimeBudget, WorkerRuntimeCapabilityPosture, WorkerRuntimeContext, WorkerRuntimePort,
...
pub(super) fn append_evidence_item(
    target: &mut Vec<EvidenceItem>,
    item: EvidenceItem,
    limit: usize,
) {
...
pub(super) fn emit_execution_governance_decision(
...
pub(super) fn trim_for_planner(input: &str, limit: usize) -> String {
        ...
        steps: loop_state
            ...
            .map(|step| PlannerTraceStep {
                ...
                decisions: vec![crate::domain::ports::PlannerDecision {
                    ...
                    query: planner_action_execution::planner_action_query(&step.action),
...
mod tests {
    ...
    fn execution_policy_gate_blocks_external_capability_before_broker_invocation() {
        ...
        let summary = super::external_capability_execution::execute_external_capability_action(
            broker.clone(),
            &crate::domain::model::default_local_execution_governance_profile(),
            Some(&policy),
            &invocation,
            super::external_capability_execution::ExternalCapabilityExecutionFrame {
    ...
    fn validate_inspect_rejects_chained_commands() {
        assert!(
            super::planner_action_execution::validate_inspect_command("ls && rm -rf /").is_err()
        );
        assert!(
            super::planner_action_execution::validate_inspect_command("cat file; echo done")
                .is_err()
        );
        assert!(
            super::planner_action_execution::validate_inspect_command("echo > /tmp/out").is_err()
        );
        assert!(
            super::planner_action_execution::validate_inspect_command("cat < /etc/passwd").is_err()
        );
        assert!(
            super::planner_action_execution::validate_inspect_command("true || false").is_err()
        );
    }

    #[test]
    fn validate_inspect_allows_safe_read_only_commands() {
        assert!(super::planner_action_execution::validate_inspect_command("git status").is_ok());
        assert!(
            super::planner_action_execution::validate_inspect_command("git remote get-url origin")
                .is_ok()
        );
        assert!(super::planner_action_execution::validate_inspect_command("ls -la").is_ok());
        assert!(super::planner_action_execution::validate_inspect_command("cat README.md").is_ok());
    }

    #[test]
    fn validate_inspect_rejects_empty_commands() {
        assert!(super::planner_action_execution::validate_inspect_command("").is_err());
        assert!(super::planner_action_execution::validate_inspect_command("   ").is_err());
    }

    #[test]
    fn planner_action_execution_maps_external_capability_sources() {
        let action = WorkspaceAction::ExternalCapability {
            invocation: ExternalCapabilityInvocation::new(
                "web.search",
                "confirm latest release notes",
                serde_json::json!({"query": "paddles"}),
            ),
        };

        assert_eq!(
            super::planner_action_execution::workspace_action_evidence_source(&action),
            "external_capability:web.search"
        );
    }

    #[test]
    fn external_capability_execution_formats_invocation_boundaries() {
        let invocation = ExternalCapabilityInvocation::new(
            "web.search",
            "confirm latest release notes",
            serde_json::json!({"query": "paddles"}),
        ...
        let rendered = super::external_capability_execution::format_external_capability_invocation(
            None,
            &invocation,
        );

        assert!(rendered.contains("fabric=web.search"));
        assert!(rendered.contains("availability=unknown"));
        assert!(rendered.contains("purpose=confirm latest release notes"));
    }
}
