...
//! phase invocations rather than chains through a stateless wrapper.
...
pub(super) async fn execute_recursive_planner_loop(
    ...
    loop {
        if let Some(control_outcome) = service
            ...
            match &decision.action {
                PlannerAction::Workspace { action } => match action {
                            ...
                            match planner_action_execution::run_planner_inspect_command(
                                ...
                                Ok(output) => {
                                    ...
                                    if !output.command_succeeded {
                                        let summary = format!("inspect failed: {}", output.summary);
                    ...
                    WorkspaceAction::ExternalCapability { invocation } => {
                        ...
                        let summary =
                            external_capability_execution::execute_external_capability_action(
                                broker,
                                context
                                    .prepared
                                    .harness_profile()
        ...
        if stop_reason.is_none() && !matches!(decision.action, PlannerAction::Stop { .. }) {
            let refinement_reason = service.mid_loop_refinement_reason(
                sequence,
                &loop_state,
            ...
            if let Some(refinement_reason) = refinement_reason
                && let Some(updated_context) = service
                ...
                && updated_context != context.interpretation
            {
                ...
                if !service.mid_loop_refinement_signature_is_stable(
                ...
                if loop_state.refinement_policy.signature_history_limit > 0 {
                    ...
                    if loop_state.refinement_signatures.len() > limit {
                        let overflow = loop_state.refinement_signatures.len().saturating_sub(limit);
...
pub(super) fn expire_turn_control_requests(
    ...
    service.expire_turn_control_requests(trace, detail);
}
