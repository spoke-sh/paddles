...
- `src/main.rs` no longer constructs `legacy_core::Instance`; prompt execution now flows through `MechSuitService` and the Sift adapter.
- `Cargo.toml` removes `legacy-core`, `legacy-provider`, and `legacy-tools` from runtime dependencies.
