...
- Build on the existing `TraceRecorder` boundary and embedded `transit-core` path. The mission must not require a networked `transit` server for normal local auto-threading.
