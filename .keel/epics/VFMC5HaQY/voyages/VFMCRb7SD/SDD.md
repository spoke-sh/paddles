...
A new `MistralRsAdapter` in `src/infrastructure/adapters/mistralrs_provider.rs` adds mistral.rs as a separate local inference provider. It uses the mistral.rs `MistralRs` struct for model loading and inference, implementing `SynthesizerEngine` by sending formatted prompts to the loaded model. The existing sift/Qwen stack is untouched — this is an additional provider option, not a replacement. Device selection (CUDA/CPU) uses mistral.rs's built-in detection.
...
| Coexistence | --provider mistralrs vs --provider sift | No breaking change; SiftAgentAdapter remains as-is |
...
src/infrastructure/adapters/
  ├── mistralrs_provider.rs   (new - MistralRsAdapter)
  ├── sift_agent_adapter.rs   (unchanged - legacy Qwen path)
  └── openai_adapter.rs       (unchanged)

main.rs routing:
  --provider mistralrs → MistralRsAdapter (mistral.rs)
  --provider sift      → SiftAgentAdapter (existing Qwen/Candle via sift)
```
...
1. User runs `paddles --provider mistralrs --model mistralai/Mistral-7B-Instruct-v0.3`
2. CLI routes to `MistralRsAdapter::new(model_id)`
