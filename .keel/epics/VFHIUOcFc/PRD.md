...
| Runtime Maintainer | An engineer evolving the recursive harness, recorder boundary, and TUI. | A generic thread contract that composes with planning, evidence gathering, and recorder/replay surfaces without adding product-specific routing hacks. |
...
- [SCOPE-03] Persist thread lineage, replies, backlinks, summaries, merge decisions, and checkpoints through the existing `TraceRecorder` boundary and embedded `transit-core` adapter.
...
| FR-03 | The first embedded transit-backed implementation must record explicit thread branch creation, thread replies, backlinks/summaries, merge decisions, and completion checkpoints with stable lineage identifiers. | GOAL-02 | must | Hidden thread state would break replay and reduce trust in the threaded harness. |
...
| NFR-04 | Embedded `transit-core` must be sufficient for the first end-to-end implementation; the design must not require a separate trace server to support local auto-threading. | GOAL-02, GOAL-04 | should | Local operators should get the feature without extra infrastructure. |
...
| Transit-backed recording and replay | Recorder tests plus replay proof | Story evidence for branch creation, thread reply, backlink/summary, merge, and checkpoint records |
...
| The current `TraceRecorder` boundary is sufficient to project thread creation and merge-back records without leaking raw transit types into domain code. | The mission may need to stop at a domain-contract refactor before full auto-threading. | Validate during story decomposition and recorder tests. |
