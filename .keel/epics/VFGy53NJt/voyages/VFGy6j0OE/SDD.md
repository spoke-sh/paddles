...
- In scope:
  ...
  - typed graph evidence/telemetry mapping
...
│     ↓                                                        │
│  Typed Paddles Evidence + Turn Events                        │
│     ↓                                                        │
│  Recursive Harness / Synthesizer Handoff                     │
└──────────────────────────────────────────────────────────────┘
          ↑                                   ↑
      Local-first models                 Operator-visible traces
```
...
| Preserve typed graph metadata, not raw upstream structs | Map graph episode/frontier/branch state into `paddles` DTOs | Avoids leaking `sift` internals through the domain |
...
3. `GraphEvidenceMapper`
   ...
   planner/evidence DTOs and turn events.
...
- `GraphEvidenceMapper`
  ...
  branch-local retained evidence as domain-friendly metadata.
...
- `TurnEvent`
  Extended with concise graph-mode summaries for the default transcript/event
  stream.
...
5. `paddles` maps those results into typed evidence items, graph-aware planner
   metadata, and operator-visible turn events.
