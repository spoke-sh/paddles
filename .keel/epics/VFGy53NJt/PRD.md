...
| GOAL-03 | Preserve graph episode/frontier/branch context as typed evidence and visible operator telemetry. | Graph-mode turns surface branch/frontier/stop metadata and branch-local evidence in the event stream and evidence bundle | Verified traces and tests |
...
| FR-03 | The Sift autonomous gatherer adapter must be able to execute graph-mode retrieval and preserve graph episode/frontier/branch information through typed `paddles` metadata rather than leaking raw upstream internals. | GOAL-02, GOAL-03 | must | Richer retrieval is only useful if the rest of the harness can see and reason over it. |
...
| FR-05 | Turn events and gathered evidence must surface graph planner summaries, branch/frontier state, and graph stop reasons in the default operator UX. | GOAL-03, GOAL-04 | must | Operators need to see the extra retrieval work the graph runtime is doing. |
| FR-06 | Foundational docs and proof artifacts must explain the graph-mode gatherer capability, config, fallback behavior, and remaining limitations. | GOAL-04 | must | The architecture shift should be legible from the docs, not only from code. |
...
| Docs and operator guidance | Doc review plus proof artifact | Updated foundational docs and a graph-mode routing proof |
...
|---------------|-------|--------|
| How much raw graph episode state should `paddles` preserve versus summarizing into a smaller domain DTO? | Runtime maintainer | Open |
...
- [ ] The gatherer boundary can select graph mode and preserve graph branch/frontier/stop metadata as typed evidence.
- [ ] Recursive retrieval can use graph-capable gatherers without introducing repository-specific routing logic.
- [ ] Foundational docs and proof artifacts explain the integration and its remaining limits clearly.
