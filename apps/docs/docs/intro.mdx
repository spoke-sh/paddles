...
sidebar_position: 1
description: Understand what Paddles is, how it turns smaller HTTP-hosted models into capable coding agents, and where to start reading.
---
...
3. **Select actions** — for planned turns, a recursive loop lets the model search, read, and refine until evidence is sufficient
4. **Render** — a separate model call produces the final answer from the evidence bundle
...
2. [Recursive Planning](./concepts/recursive-planning.mdx) — how recursive action selection works
...
5. [Steering Signals](./concepts/context-pressure.mdx) — how truncation and recursive guidance are tracked and surfaced
6. [Model Routing](./concepts/model-routing.mdx) — how action-selection and final-rendering roles are assigned
