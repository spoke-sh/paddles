...
Start a local HTTP model service such as Ollama, then select it with the
...
You can route action selection and final rendering to different HTTP model
clients:
...
The final renderer uses the remote provider while action selection uses the
local HTTP service for investigation.
