...
test('root route serves the runtime shell directly from the rust server', async ({ page }) => {
  await page.goto('/');
...
test('primary chat route submits a prompt and renders the live shared transcript', async ({ page }) => {
  await page.goto('/');
...
test('primary transit route renders significant steps and can expand to the full trace', async ({ page }) => {
  await page.goto('/');
  await page.locator('#prompt').fill('CI is failing. Can you debug it on this machine?');
  await page.locator('#send').click();
  await expect(page.locator('.msg.assistant')).toContainText(
...
test('primary manifold route boots the dedicated shell', async ({ page }) => {
  await page.goto('/');
  await page.locator('#prompt').fill('CI is failing. Can you debug it on this machine?');
  await page.locator('#send').click();
  await expect(page.locator('.msg.assistant')).toContainText(
