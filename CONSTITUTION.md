# Paddles Constitution: The Mech Suit Philosophy

> **Simulation MUST take precedence over Reality.**
>
> Foundational stack position: `4/8`
> Read this after [README.md](README.md) and before [POLICY.md](POLICY.md).

## 1. The Prime Directive
`paddles` provides a high-fidelity, local-first harness for agentic SDLC execution. Software development is treated as a rigorous simulation where intent is calibrated, execution is verified, and drift is eliminated. The recursive harness amplifies what small local models can accomplish by giving them structured tools, bounded investigation, and accumulated evidence.

## 2. Environmental Calibration
A mech suit tunes itself to its pilot and environment.
- **Credits**: The energy and inheritance allocated to a session — each turn earns its budget through bounded work.
- **Weights & Biases**: Behavioral offsets that align the agent with its mission — shaped by operator memory and the guidance graph.
- **Constitutional Bounds**: Calibration stays within safe, stable territory through schema validation, budgets, and fail-closed behavior.

## 3. Human-Agent Symbiosis
`paddles` follows the 2-queue pull model defined by Keel:
- **Humans** author the physics (ADRs, PRDs, Dogma). They define the strategic "Why" and "What".
- **Agents** execute within the physics through recursive planning. They handle the tactical "How" and implementation, earning their answers through iterative evidence gathering.
- **Verification** bridges the gap — confirming the simulation matches the requirement.

## 4. Core Invariants
Immutable principles govern the simulator's core:
- **Clean Boot**: Every session starts from a verified, structured state. The simulation's integrity is the foundation everything else builds on.
- **Local Sovereignty**: Capacity is built on local inference (`candle`), ensuring total control over the execution environment. Heavier lanes are available and degrade gracefully.

## 5. Decision Hierarchy
All moves within the mech suit follow a clear precedence:
1. **Invariants** (Immutable core principles)
2. **Constitution** (Bounds and philosophy)
3. **Architecture** (Stack and backbone design)
4. **Planning** (Specs and active missions)

---

*"The mech suit amplifies the engineer's intent through the precision of the simulation — recursive planning, grounded evidence, and visible execution."*
